...
void Detector::findLights() {
	...
	for (int i = 0;i < contours.size();i++) {
		...
		//矩形拟合
		auto r_rect = cv::minAreaRect(contours[i]);
		auto light = Light(r_rect, contours[i]);

		//轮廓分类
		if (light.ratio<R.CL_Maxratio&& light.length * light.width>R.CircleLight_area) {
		...
		else if (light.ratio < R.HL_Maxratio && light.ratio>R.HL_Minratio && light.length * light.width > R.HugeLight_area) {
